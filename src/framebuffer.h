...
enum class AttachmentType { COLOR = GL_COLOR_ATTACHMENT0, DEPTH = GL_DEPTH_ATTACHMENT, STENCIL = GL_STENCIL_ATTACHMENT };
  ...
  public:
    ...
    Texture(InternalFormat sizedInternal, InternalFormat baseInternal, PixelDataType dataType, unsigned int w, unsigned int h)
        : id(0),
          width(w),
          height(h),
          textureTarget(TextureTarget::TEX_2D),
          sizedInternalFormat(sizedInternal),
          baseInternalFormat(baseInternal),
    ...
    void createEmpty() { create(width, height, nullptr); }
    ...
    void unbind() { glBindTexture(static_cast<GLenum>(textureTarget), 0); }
...
class Framebuffer {
  public:
    Framebuffer(FramebufferTarget t) : id(0), target(t), last_attachment_count(0) { glGenFramebuffers(1, &id); }
