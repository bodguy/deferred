...
namespace obj_parser {
    using namespace glm;
    constexpr bool is_space(char x) { return x == ' ' || x == '\t'; }

    constexpr bool is_new_line(char x) { return x == '\r' || x == '\n' || x == '\0'; }
    ...
    enum class TextureFace { TEX_2D, TEX_3D_SPHERE, TEX_3D_CUBE_TOP, TEX_3D_CUBE_BOTTOM, TEX_3D_CUBE_FRONT, TEX_3D_CUBE_BACK, TEX_3D_CUBE_LEFT, TEX_3D_CUBE_RIGHT };
    ...
    struct Material {
        Material() : name(), ambient(), diffuse(), specular(), transmittance(), emission(), shininess(1.f), ior(1.f), dissolve(1.f), illum(0) { texture_map.clear(); }
    ...
    inline bool operator&(const ParseOption a, const ParseOption b) { return static_cast<ParseOption>(static_cast<unsigned int>(a) & static_cast<unsigned int>(b)) == b; }

    inline ParseOption operator|(const ParseOption a, const ParseOption b) { return static_cast<ParseOption>(static_cast<unsigned int>(a) | static_cast<unsigned int>(b)); }
    ...
    inline bool parsePrimitive(Mesh& mesh, const PrimitiveGroup& primitive, ParseOption option, const int material_id, const std::vector<vec3>& verts, const std::vector<vec2>& texcoords,
                               const std::vector<vec3>& normals, const std::string& name, const std::string& default_name) {
    ...
    inline void parseReal2(vec2& vt, const char** token, float default_x = 0.f, float default_y = 0.f) {
        vt.x = parseReal(token, default_x);
        vt.y = parseReal(token, default_y);
    }

    inline void parseReal3(vec3& vn, const char** token, float default_x = 0.f, float default_y = 0.f, float default_z = 0.f) {
    ...
    inline bool endsWith(const std::string& str, const std::string& suffix) { return str.size() >= suffix.size() && 0 == str.compare(str.size() - suffix.size(), suffix.size(), suffix); }
    ...
    inline bool loadMtl(std::vector<Material>& materials, std::unordered_map<std::string, int>& material_map, std::istream& ifs) {
        ...
        while (ifs.peek() != -1) {
            ...
            if (!line_buf.empty()) {
                if (line_buf[line_buf.size() - 1] == '\r') line_buf.erase(line_buf.size() - 1);
            }
            ...
            // bump texture
            if (((0 == strncmp(token, "map_bump", 8)) && is_space(token[8])) || ((0 == strncmp(token, "map_Bump", 8)) && is_space(token[8]))) {
                token += 9;
                Texture bump;
